...
<script type="text/javascript">
	...
	function goInsert() {
		...
		var fData = $("#frm").serialize(); // 위에 주석처리 되있는걸 한번에
		//alert(fData);
		$.ajax({
			url : "board/new",
			type : "post",
		...
		//$("#content").val("");
		//$("#writer").val("");
		$("#fclear").trigger("click"); // 취소버튼 누르면 작성중인 내용 지우기
	}
	function goContent(idx) { //idx=11,10,9
		if ($("#c" + idx).css("display") == "none") {
			...
			$("#c" + idx).css("display", "table-row"); // 펼치기
			$("#ta" + idx).attr("readonly", true);
		} else {
			$("#c" + idx).css("display", "none"); // 접기
			$.ajax({
				url : "board/count/" + idx,
				type : "put",
				dataType : "json",
...
<body>
	...
	<div class="container">
		...
		<div class="panel panel-default">
			...
			<div class="panel-footer">게시판만들기</div>
		</div>
	</div>
</body>
</html>